function EWT_Plot_Meyer_FilterBank(mfb)

%==========================================================================
% function EWT_Plot_Meyer_FilterBank(mfb,cpx)
%
...
for k=1:length(mfb)
     ang{k}=zeros(length(mfb{k}));
     ang{k}=fftshift(angle(mfb{k}));
     [~,M]=max(ang{k});
     [~,m]=min(ang{k});

     for l=1:length(ang{k})
        if (xi(l)>0) && (l>M) && (ang{k}(l)<0)
            ang{k}(l)=ang{k}(l)+2*pi;
        end
        if (xi(l)<0) && (l<m) && (ang{k}(l)>0)
            ang{k}(l)=ang{k}(l)-2*pi;
        end
